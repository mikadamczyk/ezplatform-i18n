...
class SplitLanguageCommitsCommand extends Command
{
    ...
    protected static $defaultName = 'app:split-language-commits';

    protected function configure(): void
    {
        $this
            ->setDescription('Analyzes changes from l10n_master against the current branch
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        $crowdinApiKey = getenv('CROWDIN_API_KEY');

        if (!$crowdinApiKey) {
            $output->writeln('Environment variable CROWDIN_API_KEY must be set for the ezplatform project');

            return 1;
        }

        try {
            //$translations = $this->getLanguagesData($projectName, $crowdinApiKey);
        } catch (\Exception $e) {
     ...
     * @param string $crowdinApiKey
     *
     * @return array
     * @throws \Exception
     */
    private function getLanguagesData(string $projectName, string $crowdinApiKey)
    {
        $languagesData = [];
        $languagesMap = self::LANGUAGES_MAP;
        $statuses = $this->getCrowdinStatuses($projectName, $crowdinApiKey);
     ...
     * @throws \Exception
     */
    private function getCrowdinStatuses(string $projectName, string $crowdinApiKey): array
    {
        $statuses = json_decode(
            file_get_contents(
                sprintf('https://api.crowdin.com/api/project/%s/status?key=%s&json', $projectName, $crowdinApiKey)
            )
        );
